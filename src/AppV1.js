...
function MovieDetails({ selectedId, onCloseMovie, onAddWatched, watched }) {
  ...
  /**
   * Trying to define the state conditionally -> which is ideally wrong as it break the structure or order of hook in the backend
   *
   * when the condition become true and useState is creted the application crashes with error that there is change in order of Hook
   *
   * here hook means all -> useState and useEffect ->  both order get disturbed
   *
   * In backend -> Linked list of hook is created
   *
   */
  // if( imdbRating > 8 ) {
   ...
   * IMPORTANT POINT
   * 
   * whatever we pass to useState is the initial state and react only look this initial state on initial render ( first time Mount )
   * 
   * const [isTop, setIsTop] = useState(imdbRating > 0);
   * 
   * if on first render isTop is fale -> this will stay false even if in second condition the given condn become true
   * 
   * Reason is given above -> that useState take the value in initial render
   * 
   * Way to fix -> set it inside ueEffect
   * 
   * useEffect(() => {
   *    setIsTop(imdbRating)
   * }, [imdbRating])
   * 
   * 
   * If we check the above scenario -> the best way to do that using DERIVED STATE
   * 
   * const isTop = imdbRating > 8 ; -> this variable get regenerated after every render
   * 
   */
